...
function makeCanRow(name: string, pct: number, totalMg: number, effect: string): CannabinoidRow {
  ...
  return { name, pct: pct * 100, mg30ml, mgMl: +(mg30ml / 30).toFixed(1), mgDrop: +(mg30ml / 900).toFixed(3), effect };
}
...
export const PRODUCTS: Product[] = [
  {
    ...
    cbdPct: 0.65,
    cbdMg: 4704,
    description: "7237 mg/30 mL · 241 mg/mL · 30 gotas/mL",
  ...
  // ── LINHA ESSENTIAL — Segunda opção / alternativa de entrada ──
  {
    ...
    cbdPct: 0.972,
    cbdMg: 6346,
    description: "6525 mg/30 mL · 200 mg/mL · 30 gotas/mL · 6,7 mg can./gota",
    ...
    cannabinoids: [
      { name: "CBD", pct: 97.2, mg30ml: 6346, mgMl: +(6346 / 30).toFixed(1), mgDrop: +(6346 / 900).toFixed(3), effect: "Ansiolítico, anticonvulsivante, neuroprotetor" },
      { name: "CBG", pct: 2.7, mg30ml: 179, mgMl: +(179 / 30).toFixed(1), mgDrop: +(179 / 900).toFixed(3), effect: "Antibacteriano, neuroprotetor" },
    ],
  ...
  },
  {
    ...
    cbdPct: 0.963,
    cbdMg: 6846,
    description: "7109 mg/30 mL · 200 mg/mL · 30 gotas/mL · 6,7 mg can./gota · THC ≤ 0,3%",
